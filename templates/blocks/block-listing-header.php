	...
	if ( get_post_type() == 'listing' ) {
		...
		if ( $terms && !is_wp_error( $terms ) ) {
			foreach ( $terms as $term ) {
				?>
				<a class="property-type<?php echo ($size == 'listing_list') ? ' hide-for-small' : ''; ?>" href="<?php echo get_term_link( $term->slug, "property-type" ); ?>" <?php the_ga_event('Cottage List', 'Pick category', 'Grid item'); ?>><?php echo $term->name; ?></a>
				<?php
